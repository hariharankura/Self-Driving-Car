...
void test_initialize_speed_sensor_interrupt(void) {
  gpio_s gpio_1;
...
void test_calculate_rpm_and_mph(void) {
  ...
  float test_mph = ((pi * wheel_diameter_in_cm) * (test_rpm * 60)) / miles_to_cm;
  printf("mph:%f\n", test_mph);
  ...
  test_mph = ((pi * wheel_diameter_in_cm) * (test_rpm * 60)) / miles_to_cm;
  TEST_ASSERT_EQUAL_UINT32(test_rpm, get_rpm());
  TEST_ASSERT_EQUAL_FLOAT(test_mph, get_mph());
}
...
// void test_get_windowtime(void) { TEST_ASSERT_EQUAL(300, get_windowtime()); }